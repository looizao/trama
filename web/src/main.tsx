...
function ClientPage() {
  ...
  return <div className="page"><Link to="/clients" className="back">{t("← All clients")}</Link>{client.isPending ? <Loading /> : client.error ? <Banner text={client.error.message} /> : <><div className="page-heading"><div><span className="eyebrow">{t("CLIENT WORKSPACE")}</span><h1>{client.data.name}</h1><p>{client.data.email || t('A private space for this client’s look and hair journey.')}</p></div><button className="button primary" onClick={() => setShowForm(v => !v)}>{t("+ New journey")}</button></div>{client.data.notes && <div className="note"><strong>{t("Private notes")}</strong><p>{client.data.notes}</p></div>}{showForm && <section className="card form-card"><h2>{t("Start a journey")}</h2><form onSubmit={add} className="form-grid"><label className="wide">{t("Journey name")}<input value={title} onChange={e => setTitle(e.target.value)} placeholder={t("Spring transformation")} required minLength={2} /></label><label className="wide">{t("Description")}<textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} placeholder={t("Your goals for this consultation")} /></label><Banner text={action.error} /><div className="form-actions"><button type="button" className="button ghost" onClick={() => setShowForm(false)}>{t("Cancel")}</button><button className="button primary" disabled={action.busy}>{t("Create journey")}</button></div></form></section>}<div className="section-toolbar"><h2>{t("Journeys ")}<span className="count">{cases.data?.length ?? 0}</span></h2></div>{cases.isPending ? <Loading /> : cases.data?.length ? <div className="journey-list">{cases.data.map(c => <Link key={c.id} to="/cases/$caseId" params={{ caseId: c.id }} className="journey-card"><div className="journey-symbol">✳</div><div><h3>{c.title}</h3><p>{c.description || t('Explore looks and document progress')}</p><small>{t("Started ")}{dateLabel(c.createdAt, locale)}</small></div><span className="arrow">↗</span></Link>)}</div> : <Empty title={t("No journeys yet")} text={t("Create a journey to collect photos, generate looks, and show progress.")} />}</>}</div>
}
...
function CasePage() {
  ...
  async function uploadImage(file?: File) { if (!file) return; const form = new FormData(); form.append('image', file); await upload.run(() => api(`/cases/${caseId}/assets`, { method: 'POST', body: form })) }
	async function generate(e: React.FormEvent) { e.preventDefault(); await generation.run(() => api(`/cases/${caseId}/runs`, json('POST', { sourceAssetId, prompt, quantity, modelId: models.data?.models[0]?.id ?? '' })), () => setPrompt('')) }
