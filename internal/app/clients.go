...
func (a *App) listClients(w http.ResponseWriter,r *http.Request) {
	...
	if err != nil { problem(w,500,"could not load clients"); return }; defer rows.Close()
	items := []ClientRecord{}
	for rows.Next() { var x ClientRecord; if err=rows.Scan(&x.ID,&x.Name,&x.Email,&x.Notes,&x.CreatedAt); err != nil { problem(w,500,"could not load clients"); return }; items=append(items,x) }
	respond(w,200,items)
}

func (a *App) createClient(w http.ResponseWriter,r *http.Request) {
	var in struct { Name string `json:"name"`; Email string `json:"email"`; Notes string `json:"notes"` }
	if err:=decodeJSON(r,&in); err!=nil { badRequest(w,err); return }
	in.Name = strings.TrimSpace(in.Name); in.Email = strings.TrimSpace(in.Email)
	if len(in.Name)<2 || len(in.Name)>160 || len(in.Email)>254 || len(in.Notes)>10000 { problem(w,400,"invalid client fields"); return }
	u:=userFrom(r); x:=ClientRecord{ID:newID(),Name:in.Name,Email:in.Email,Notes:in.Notes,CreatedAt:now()}
	...
	if err!=nil { problem(w,500,"could not create client"); return }
	a.audit(r,"create","client",x.ID); respond(w,201,x)
}

func (a *App) getClient(w http.ResponseWriter,r *http.Request) {
	id:=r.PathValue("clientID"); if !isID(id) { problem(w,404,"client not found"); return }
	var x ClientRecord
	...
	if err!=nil { problem(w,404,"client not found"); return }; respond(w,200,x)
}

func (a *App) updateClient(w http.ResponseWriter,r *http.Request) {
	id:=r.PathValue("clientID"); if !a.clientExists(r,id) { problem(w,404,"client not found"); return }
	var in struct { Name string `json:"name"`; Email string `json:"email"`; Notes string `json:"notes"` }
	if err:=decodeJSON(r,&in); err!=nil { badRequest(w,err); return }
	in.Name=strings.TrimSpace(in.Name); in.Email=strings.TrimSpace(in.Email)
	if len(in.Name)<2 || len(in.Name)>160 || len(in.Email)>254 || len(in.Notes)>10000 { problem(w,400,"invalid client fields"); return }
	...
	if err!=nil { problem(w,500,"could not update client"); return }; a.audit(r,"update","client",id); a.getClient(w,r)
}

func (a *App) listCases(w http.ResponseWriter,r *http.Request) {
	clientID:=r.PathValue("clientID"); if !a.clientExists(r,clientID) { problem(w,404,"client not found"); return }
	...
	if err!=nil { problem(w,500,"could not load cases"); return }; defer rows.Close()
	items:=[]CaseRecord{}; for rows.Next() { var x CaseRecord; if err=rows.Scan(&x.ID,&x.ClientID,&x.Title,&x.Description,&x.CreatedAt); err!=nil { problem(w,500,"could not load cases"); return }; items=append(items,x) }
	respond(w,200,items)
}
...
func (a *App) createCase(w http.ResponseWriter,r *http.Request) {
	...
	if err:=decodeJSON(r,&in); err!=nil { badRequest(w,err); return }
	in.Title=strings.TrimSpace(in.Title); if len(in.Title)<2 || len(in.Title)>160 || len(in.Description)>10000 { problem(w,400,"invalid case fields"); return }
	...
	if err!=nil { problem(w,500,"could not create case"); return }; a.audit(r,"create","case",x.ID); respond(w,201,x)
}

func (a *App) getCase(w http.ResponseWriter,r *http.Request) {
	id:=r.PathValue("caseID"); if !isID(id) { problem(w,404,"case not found"); return }
	var x CaseRecord
	...
	if err!=nil { problem(w,404,"case not found"); return }; respond(w,200,x)
}

func (a *App) listMilestones(w http.ResponseWriter,r *http.Request) {
	id:=r.PathValue("caseID"); if !a.caseExists(r,id) { problem(w,404,"case not found"); return }
	...
	if err!=nil { problem(w,500,"could not load milestones"); return }; defer rows.Close()
	items:=[]Milestone{}; for rows.Next() { var x Milestone; if err=rows.Scan(&x.ID,&x.CaseID,&x.Title,&x.Position,&x.CreatedAt); err!=nil { problem(w,500,"could not load milestones"); return }; items=append(items,x) }
	respond(w,200,items)
}

func (a *App) createMilestone(w http.ResponseWriter,r *http.Request) {
	id:=r.PathValue("caseID"); if !a.caseExists(r,id) { problem(w,404,"case not found"); return }
	var in struct { Title string `json:"title"` }; if err:=decodeJSON(r,&in); err!=nil { badRequest(w,err); return }
	in.Title=strings.TrimSpace(in.Title); if len(in.Title)<2 || len(in.Title)>160 { problem(w,400,"invalid milestone title"); return }
	...
	if err!=nil { problem(w,500,"could not create milestone"); return }; a.audit(r,"create","milestone",x.ID); respond(w,201,x)
}
