...
func Open(ctx context.Context) (*App, error) {
	...
	for _, statement := range strings.Split(schema,";") {
		...
		if _, err = pool.Exec(ctx, statement); err != nil { pool.Close(); return nil, fmt.Errorf("schema: %w", err) }
	}
	store, err := OpenStorage(ctx)
	if err != nil { pool.Close(); return nil, err }
	a := &App{DB: pool, Storage: store, FalKey: os.Getenv("FAL_KEY")}
	if err := a.bootstrapAdmin(ctx); err != nil { pool.Close(); return nil, err }
	return a, nil
}

func (a *App) ConnectTemporal() error {
	address := os.Getenv("TEMPORAL_ADDRESS")
	if address == "" { return errors.New("TEMPORAL_ADDRESS is required") }
...
func (a *App) Close() {
	if a.Temporal != nil { a.Temporal.Close() }
	if a.DB != nil { a.DB.Close() }
}

func (a *App) bootstrapAdmin(ctx context.Context) error {
	var count int
	if err := a.DB.QueryRow(ctx, "SELECT count(*) FROM users").Scan(&count); err != nil { return err }
	if count > 0 { return nil }
	email := strings.ToLower(strings.TrimSpace(os.Getenv("ADMIN_EMAIL")))
	password := os.Getenv("ADMIN_PASSWORD")
	if email == "" || password == "" { return errors.New("ADMIN_EMAIL and ADMIN_PASSWORD are required for the first launch") }
	if len(password) < 12 { return errors.New("ADMIN_PASSWORD must have at least 12 characters") }
	...
	if _, err = tx.Exec(ctx, "INSERT INTO organizations (id,name) VALUES ($1,$2)", orgID, "My studio"); err != nil { return err }
	if _, err = tx.Exec(ctx, "INSERT INTO users (id,organization_id,email,name,password_hash,role) VALUES ($1,$2,$3,$4,$5,'admin')", userID, orgID, email, "Administrator", hash); err != nil { return err }
