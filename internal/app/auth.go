...
func hashPassword(password string) (string, error) {
	if len(password) < 12 { return "", errors.New("password must have at least 12 characters") }
	salt := make([]byte, 16)
...
func secureCookie() bool { return strings.HasPrefix(os.Getenv("PUBLIC_BASE_URL"), "https://") || os.Getenv("RENDER_EXTERNAL_URL") != "" }
...
func (a *App) setSession(w http.ResponseWriter, r *http.Request, userID string) error {
	...
	if _, err = a.DB.Exec(r.Context(), "INSERT INTO sessions (token_hash,user_id,expires_at) VALUES ($1,$2,$3)", digest, userID, expires); err != nil { return err }
...
func (a *App) authenticated(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil { problem(w, 401, "sign in required"); return }
		...
		if err != nil { clearSession(w); problem(w, 401, "session expired"); return }
...
func requireAdmin(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if userFrom(r).Role != "admin" { problem(w, 403, "admin access required"); return }
		next(w,r)
	}
}

type attemptCounter struct { mu sync.Mutex; attempts map[string][]time.Time }
...
func (c *attemptCounter) allowed(key string) bool {
	...
	for _, t := range c.attempts[key] { if t.After(cutoff) { recent = append(recent,t) } }
	if len(recent) >= 8 { c.attempts[key] = recent; return false }
...
func (a *App) login(w http.ResponseWriter, r *http.Request) {
	...
	if err := decodeJSON(r, &input); err != nil { badRequest(w,err); return }
	email := strings.ToLower(strings.TrimSpace(input.Email))
	if len(email) > 254 || len(input.Password) > 1024 { problem(w,400,"invalid credentials"); return }
	if !loginAttempts.allowed(r.RemoteAddr+":"+email) { problem(w,429,"too many attempts; try again later"); return }
	var id, hash string
	err := a.DB.QueryRow(r.Context(), "SELECT id,password_hash FROM users WHERE email=$1 AND active=true", email).Scan(&id,&hash)
	if err != nil || !verifyPassword(hash,input.Password) { problem(w,401,"invalid credentials"); return }
	if err := a.setSession(w,r,id); err != nil { problem(w,500,"could not create session"); return }
...
func (a *App) changePassword(w http.ResponseWriter, r *http.Request) {
	var input struct { Current string `json:"current"`; Next string `json:"next"` }
	if err := decodeJSON(r,&input); err != nil { badRequest(w,err); return }
	u := userFrom(r)
	var stored string
	if err := a.DB.QueryRow(r.Context(),"SELECT password_hash FROM users WHERE id=$1",u.ID).Scan(&stored); err != nil { problem(w,500,"could not load account"); return }
	if !verifyPassword(stored,input.Current) { problem(w,401,"current password is incorrect"); return }
	hash, err := hashPassword(input.Next)
	if err != nil { badRequest(w,err); return }
	if _, err = a.DB.Exec(r.Context(),"UPDATE users SET password_hash=$1 WHERE id=$2",hash,u.ID); err != nil { problem(w,500,"could not update password"); return }
