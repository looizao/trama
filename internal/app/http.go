...
func (a *App) Handler() http.Handler {
	...
	mux.HandleFunc("GET /api/clients/{clientID}/cases", a.authenticated(a.listCases))
	mux.HandleFunc("POST /api/clients/{clientID}/cases", a.authenticated(a.createCase))
	mux.HandleFunc("GET /api/cases/{caseID}", a.authenticated(a.getCase))
	mux.HandleFunc("GET /api/cases/{caseID}/milestones", a.authenticated(a.listMilestones))
	mux.HandleFunc("POST /api/cases/{caseID}/milestones", a.authenticated(a.createMilestone))
	mux.HandleFunc("GET /api/cases/{caseID}/assets", a.authenticated(a.listAssets))
	mux.HandleFunc("POST /api/cases/{caseID}/assets", a.authenticated(a.uploadAsset))
	...
	mux.HandleFunc("GET /api/cases/{caseID}/runs", a.authenticated(a.listRuns))
	mux.HandleFunc("POST /api/cases/{caseID}/runs", a.authenticated(a.createRun))
	...
	return http.HandlerFunc(func(w http.ResponseWriter,r *http.Request) {
		if r.Method != http.MethodGet && r.Method != http.MethodHead && r.Method != http.MethodOptions {
			...
			if origin != "" {
				...
				if base == "" { base = "http://"+r.Host; if r.TLS != nil { base = "https://"+r.Host } }
				if origin != strings.TrimSuffix(base,"/") { problem(w,403,"invalid request origin"); return }
			}
		}
		defer func() { if recovered := recover(); recovered != nil { log.Printf("panic: %v",recovered); problem(w,500,"internal error") } }()
		mux.ServeHTTP(w,r)
	})
}

func (a *App) serveWeb(w http.ResponseWriter,r *http.Request) {
	if r.Method != "GET" && r.Method != "HEAD" { problem(w,404,"not found"); return }
	if strings.HasPrefix(r.URL.Path,"/api/") { problem(w,404,"not found"); return }
	path := filepath.Join("web/dist", filepath.Clean("/"+r.URL.Path))
	if info, err := os.Stat(path); err == nil && !info.IsDir() {
		if strings.HasPrefix(r.URL.Path,"/assets/") { w.Header().Set("Cache-Control","public, max-age=31536000, immutable") }
...
func (a *App) audit(r *http.Request, action, kind, id string) {
	...
	if err != nil { log.Printf("audit failed: %v",err) }
}

func (a *App) models(w http.ResponseWriter,r *http.Request) {
	respond(w,200,map[string]any{"enabled":a.FalKey!="" && a.Temporal!=nil && a.Storage.mode=="s3","models":[]map[string]string{{"id":"fal-ai/flux-pro/kontext","name":"FLUX.1 Kontext Pro","capability":"Reference image editing"}}})
}
