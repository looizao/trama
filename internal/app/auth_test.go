...
func TestPasswordAndSessionSecrets(t *testing.T) {
	...
	if strings.Contains(hash, "a strong example password") {
		t.Fatal("password stored in hash")
	}
	...
	if first == second {
		t.Fatal("session token repeated")
	}
}
...
func TestModelAssetLinkRejectsTamperingAndExpiry(t *testing.T) {
	...
	if err != nil || key != "org/case/photo.jpg" {
		t.Fatalf("valid link rejected: %v", err)
	}
	if _, err = store.VerifyModelToken(token + "x"); err == nil {
	...
	if _, err = store.VerifyModelToken(strings.TrimPrefix(expired, "https://trama.example/api/model-assets/")); err == nil {
